...
export default function MarketArea({
  ...
  const deckPreviewTimer = useRef(null);
  ...
  const previewCardSize =
    "w-[150px] h-[197px] sm:w-[190px] sm:h-[250px] md:w-[230px] md:h-[300px]";
      ...
      >
        ...
        <div className="flex flex-row items-center justify-between min-h-[6.2rem] sm:min-h-[7.5rem] md:min-h-[8.5rem] p-2 bg-white dark:bg-gray-900 rounded border border-gray-300 dark:border-gray-700 w-full transition-colors relative overflow-hidden">
          <button
            ...
            title="Cava"
            className="flex flex-col items-center justify-center flex-shrink-0 w-[52px] sm:w-[68px] md:w-[80px] lg:w-[88px] ml-0.5 sm:ml-2 focus:outline-none hover:scale-105 transition-transform"
          >
        ...
        {showDeckPreview && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.7, y: 20 }}
              className="bg-black/30 rounded-2xl p-6 shadow-2xl"
            >
              <Card type="deck" sizeClassName={previewCardSize} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
